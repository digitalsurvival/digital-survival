...
src_compile() {
	cd "${S}/build/gcc_linux"
	# Currently build fails because the script attempts to start papaya upon completion
	# It creates a sandbox vioation since it tries to access /dev/nvidiactl directly
	/bin/sh build.sh || die "build failed!"
}

src_install() {
	doexe "${S}/build/gcc_2015/papaya"
}
